<hr class="mt-12"/>
<h2 class="font-black text-3xl py-2.5 text-green-300">Agenda</h2>
<hr class="mb-6 w-28 shadow"/>

@foreach($agenda as $event)
    <div class="card shadow w-11/12 flex">
        <div class="bg-green-300 text-center">
            <p class="text-3xl font-light text-white py-5 px-1.5 w-20">01 <span class="uppercase font-black">oct</span></p>
        </div>
        <div class="pt-2.5 pl-2.5">
            <h2 class="uppercase font-black text-2xl leading-7">{{$event['title']}}</h2>
            <p class="text-sm font-normal leading-tight py-0.5">Projet de fresque avec La HELHA à La Louvière.</p>
        </div>
    </div>
@endforeach

{{--<div class="card shadow w-11/12 flex">--}}
{{--    <div class="bg-green-300 text-center">--}}
{{--        <p class="text-3xl font-light text-white py-5 px-1.5 w-20">14 <span class="uppercase font-black">sep</span></p>--}}
{{--    </div>--}}
{{--    <div class="pt-2.5 pl-2.5">--}}
{{--        <h2 class="uppercase font-black text-2xl leading-7">Mercredi s’éveille</h2>--}}
{{--        <p class="text-sm font-normal leading-tight py-0.5">Ils découvriront différentes techniques lors de cette nouvelle saison. Dessin, peinture, volume, stopmotion...</p>--}}
{{--    </div>--}}
{{--</div>--}}
